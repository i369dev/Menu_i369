...
export const ClientManager: React.FC = () => {
    ...
    const [tempClient, setTempClient] = useState<any>({});
    const [originalClient, setOriginalClient] = useState<any>({});

    const isClientDirty = JSON.stringify(tempClient) !== JSON.stringify(originalClient);

    const startEditClient = (client?: any, index?: number) => {
        if (client && index !== undefined) {
            setEditingClientId(index);
    ...
    const saveClient = () => {
        ...
        if (editingClientId === -1) {
            newClients.push(tempClient);
        } else {
            if (editingClientId !== null) newClients[editingClientId] = tempClient;
        }
    ...
    const deleteClient = (index: number) => {
        if (confirmDelete("Remove client?")) setTrustedClients(trustedClients.filter((_, i) => i !== index));
    };

    const toggleVisibility = (index: number, visible: boolean) => {
        const newClients = [...trustedClients];
        newClients[index] = { ...newClients[index], isVisible: visible };
        setTrustedClients(newClients);
    };
        ...
        <div className="max-w-4xl mx-auto pb-20">
                {editingClientId === null ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-4"><Button onClick={saveClient} variant="success" disabled={!isClientDirty}>Save</Button><Button onClick={() => setEditingClientId(null)} variant="secondary">Cancel</Button></div>
                    </div>
                    </Card>
            )}
        </div>
